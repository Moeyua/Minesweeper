...
//随机生成炸弹
var creatBomb = function (bomb) {
  var bombFactor = Math.random();
  if (bombFactor < 0.2) {
    bomb.setAttribute('isBomb', 'true');
  } else {
    bomb.setAttribute('isBomb', 'false');
  }
  ...
  //屏蔽右键菜单
  bomb.oncontextmenu = function (event) {
    event.preventDefault();
  };

  //绑定鼠标右击事件
  bomb.addEventListener('contextmenu', function () {
    contextmenu(this.id);
  }, false)
};

//生成提示数字
var bombsAround = function (row) {
  var bombs = document.getElementsByClassName('bomb');
  for (var i = 0; i < bombs.length; i++) {
    var table = document.getElementById('gameui');
    if (bombs[i].getAttribute('isbomb') === 'true') {
      table.setAttribute('bombs', parseInt(table.getAttribute('bombs')) + 1);
    }
    var count = 0;

    if (i <= row - 1) { //第一行
      if (i === 0) {
        if (bombs[i + 1].getAttribute('isbomb') == 'true') {
          count++
        } //右
        if (bombs[i + row].getAttribute('isbomb') == 'true') {
          count++
        } //下
        if (bombs[i + row + 1].getAttribute('isbomb') == 'true') {
          count++
        } //右下
      } else if (i === row - 1) {
        if (bombs[i - 1].getAttribute('isbomb') == 'true') {
    ...
    // 生成数据
    bombs[i].setAttribute('bombNotice', count);
  }
};
// 扩散动画
var opean = function (id) {
  var id = parseInt(id);
  var bomb = document.getElementById(id);
  var flag = bomb.getAttribute('flag');
  if (flag === '0') {
    //状态变为打开
    bomb.setAttribute('flag', '1');
    //设置提示
    if(bomb.getAttribute('bombNotice') !== '0'){
      bomb.innerText = bomb.getAttribute('bombNotice');
    }
    //设置背景字体
    bomb.setAttribute('style',
      'background-color:#40407a;' + 'color:white;' +
      'box-shadow:inset 4px 4px 8px #363668, inset -4px -4px 8px #4a4a8c;'
    );
    //变为不可选中状态
    bomb.setAttribute('disabled', 'disabled');
...
//弹窗
var alterWindow = function (innerText) {
  var alter = document.getElementById('alter');
  alter.firstElementChild.firstElementChild.innerText = innerText;
  setTimeout(function () {
    alter.hidden = false;
  }, 500);
}
...
var click = function (id) {
  id = parseInt(id);
  var bomb = document.getElementById(id);
  var isbomb = bomb.getAttribute('isbomb');
  //是炸弹，游戏结束
  if (isbomb === 'true') {
    //显示所有炸弹
    bomb.setAttribute('style',
  ...
  //不是炸弹，且未被打开
  else if (bomb.getAttribute('flag') === '0') {
    opean(id);
  }
}
...
var contextmenu = function (id) {
  ...
  var table = document.getElementById('gameui');
  //如果原先没有标记
  if (bomb.getAttribute('flag') === '0') {
    //添加标记
    bomb.setAttribute('flag', '2');
    bomb.innerHTML = '<svg t="1597062947705" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="11706" width="32" height="32"><path d="M119.220948 0.046078A42.420948 42.420948 0 0 0 76.8 42.467026v939.112026a42.420948 42.420948 0 0 0 42.420948 42.420948 42.420948 42.420948 0 0 0 42.420948-42.420948V42.467026A42.420948 42.420948 0 0 0 119.220948 0.046078zM757.729358 87.813557A405.924591 405.924591 0 0 1 568.297883 43.929817 404.461799 404.461799 0 0 0 378.866407 0.046078a402.267612 402.267612 0 0 0-189.431476 57.048861v481.258344a402.267612 402.267612 0 0 1 189.431476-57.048862 406.655986 406.655986 0 0 1 189.431476 43.88374 403.730404 403.730404 0 0 0 189.431475 43.883739A403.730404 403.730404 0 0 0 947.160834 512.023039V28.570509a402.999008 402.999008 0 0 1-189.431476 59.243048z" fill="#b33939" p-id="11707"></path></svg>'
    //如果是炸弹，未排除炸弹减一
    if (bomb.getAttribute('isbomb') === 'true') {
      table.setAttribute('bombs', parseInt(table.getAttribute('bombs')) - 1);
    }
    //如果不是炸弹，未排除炸弹加一
    else {
    ...
    //bomb.setAttribute('style', 'background-color:#b33939;');
  }
  //如果原先有标记
  else if (bomb.getAttribute('flag') === '2') {
    bomb.setAttribute('flag', '0');
  ...
  if (parseInt(table.getAttribute('bombs')) === 0) {
    alterWindow('YOU WIN!');
    'GAMEOVER'
    'YOU WIN!'
  }
...
//生成表格
var creatTable = function (row) {
  var table = document.createElement('table');
  //给游戏添加胜利条件
  table.setAttribute('id', 'gameui');
  table.setAttribute('bombs', 0);
  for (r = 0; r < row; r++) {
...
var gameUI = function () {
  ...
  //2、将table和exit按钮添加至DocumentFragment中
  docfrag.appendChild(creatTable(row));
  //3、将DocumentFragment的所有子节点移动至源文档内
